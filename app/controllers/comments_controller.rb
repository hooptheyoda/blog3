...
before_action :authenticate_user!
  ...
  def create
    ...
    @comment.id = User.find(params[:user_id])
    @comment.commenter = User.find(params[:username])
    respond_to do |format|
      if @comment.save
        ...
        format.json { render json: article_path(@article).errors, status: :unprocessable_entity }
      end
    end
  end
